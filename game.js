...
const OPPONENTS = [
  { name: 'GIGI', animal: 'giraffe', avatar: 'assets/gigi-giraffe.png', position: '50% 50%' },
  { name: 'MIMI', animal: 'hamster', avatar: 'assets/mimi-hamster.png', position: '50% 20%' },
  { name: 'BOBO', animal: 'monkey', avatar: 'assets/bobo-monkey.png', position: '50% 18%' }
];
...
const state = {
  ...
  streak: 0, bestStreak: 0, times: [], locked: true, sound: true, duration: 12,
  timeLeft: 12, startedAt: 0, timer: null, nextTimer: null, opponent: null, audio: null
};
...
function fallbackAvatar(opponent) {
  const emoji = opponent.name === 'GIGI' ? '🦒' : opponent.name === 'MIMI' ? '🐹' : '🐵';
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 160"><rect width="160" height="160" rx="80" fill="#fbf6ec"/><text x="80" y="111" text-anchor="middle" font-size="96">${emoji}</text></svg>`;
...
function spinReel(opponent) {
  ...
  track.innerHTML = sequence.map(item => `<div class="reel-face" style="height:${height}px"><img src="${item.avatar}" alt="${item.name}" style="object-position:${item.position}"></div>`).join('');
...
function beginMatchmaking(avoidCurrent = false) {
  ...
  $('#matchTitle').textContent = 'Searching for a rival…';
  $('#matchSubtitle').textContent = 'Scanning the OKA arena for a fun match!';
  ...
  setTimeout(() => {
    ...
    $('#matchSubtitle').textContent = `Perfect match! IREM vs ${chosen.name}`;
...
function resolveRound(playerButton) {
  ...
  if (playerButton) {
    playerButton.classList.add('player-pick');
    addMarker(playerButton, 'IREM', 'player-marker');
  }
...
function choose(button) { resolveRound(button); }
function timeout() { if (!state.locked) resolveRound(null); }
